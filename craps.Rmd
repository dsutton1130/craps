...
# Running a contest between Ishan & Mark 100,000 times.

contest_size <- 6
number_of_contests <- 1000
n <- contest_size * number_of_contests

contest <- tibble(player_Ishan = map_int(1:n, ~ roll_two_dice()),
                   player_John_Mark = map_int(1:n, ~roll_two_dice())) %>%
     mutate(IB_craps = player_Ishan %in% c(7,11)) %>%
     mutate(JM_craps = player_John_Mark %in% c(7,11)) %>%
     mutate(winner = "Tie") %>%
